from sqlalchemy import Column, Integer, String, Text, DateTime, Boolean, ForeignKey, Table, CheckConstraint, UniqueConstraint, Index
...
user_connections = Table(
    'user_connections',
    Base.metadata,
    Column('requester_id', Integer, ForeignKey('users.id', ondelete='CASCADE'), primary_key=True),
    Column('addressee_id', Integer, ForeignKey('users.id', ondelete='CASCADE'), primary_key=True),
...
post_mentions = Table(
    'post_mentions',
    Base.metadata,
    Column('post_id', Integer, ForeignKey('social_posts.id', ondelete='CASCADE'), primary_key=True),
...
class SocialPost(Base):
    ...
    mentions = relationship("User", secondary=post_mentions, back_populates="mentioned_in_posts")
    reports = relationship("ContentReport", back_populates="post", cascade="all, delete-orphan")
...
class PostComment(Base):
    ...
    user_id = Column(Integer, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    parent_comment_id = Column(Integer, ForeignKey('post_comments.id'), index=True)  # For threaded replies
...
class EventReview(Base):
    ...
    user_id = Column(Integer, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    booking_id = Column(Integer, ForeignKey('bookings.id'), index=True)  # If review is based on attendance
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint('rating >= 1 AND rating <= 5', name='event_review_rating_check'),
        CheckConstraint('organization_rating IS NULL OR (organization_rating >= 1 AND organization_rating <= 5)', name='organization_rating_check'),
        CheckConstraint('value_rating IS NULL OR (value_rating >= 1 AND value_rating <= 5)', name='value_rating_check'),
        CheckConstraint('venue_rating IS NULL OR (venue_rating >= 1 AND venue_rating <= 5)', name='venue_rating_check'),
        CheckConstraint('content_rating IS NULL OR (content_rating >= 1 AND content_rating <= 5)', name='content_rating_check'),
        CheckConstraint('atmosphere_rating IS NULL OR (atmosphere_rating >= 1 AND atmosphere_rating <= 5)', name='atmosphere_rating_check'),
...
class UserConnection(Base):
    ...
    status = Column(SQLEnum(ConnectionStatus), default=ConnectionStatus.PENDING, index=True)
    connection_message = Column(Text)  # Optional message with request
    
    # Connection metadata
    connection_strength = Column(Integer, default=1)  # Algorithm-based connection strength
...
class EventAttendance(Base):
    ...
    # Attendance status
    attendance_status = Column(String(20), default='going')  # going, interested, not_going
    visibility = Column(String(20), default='public')  # public, friends, private
...
class SocialNotification(Base):
    ...
    event_id = Column(Integer, ForeignKey('events.id'), index=True)
    connection_id = Column(Integer, ForeignKey('user_social_connections.id'), index=True)
...
class ContentReport(Base):
    ...
    action_taken = Column(String(100))  # content_removed, user_warned, user_suspended, etc.
...
class HashtagTrend(Base):
    ...
    # Constraints
    __table_args__ = (
        Index('idx_hashtag_trending', 'is_trending', 'trend_score'),
    )
