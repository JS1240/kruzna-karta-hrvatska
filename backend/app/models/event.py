from sqlalchemy import Column, Integer, String, Date, DateTime, Text, Boolean, Numeric, ForeignKey, CheckConstraint
...
class Event(Base):
    ...
    approval_status = Column(String(20), default='pending', index=True)  # pending, approved, rejected
    platform_commission_rate = Column(Numeric(5, 2), default=5.0)  # 5% default commission
    ...
    favorited_by = relationship("User", secondary="user_favorites", back_populates="favorite_events")
    ...
    event_organizer = relationship("User", foreign_keys=[organizer_id], back_populates="organized_events")
    
    # Constraints
    __table_args__ = (
        CheckConstraint('date >= CURRENT_DATE - INTERVAL \'1 year\'', name='events_date_check'),
        CheckConstraint('end_date IS NULL OR end_date >= date', name='events_end_date_check'),
        CheckConstraint('event_status IN (\'active\', \'cancelled\', \'postponed\', \'sold_out\', \'draft\')', name='events_status_check'),
        CheckConstraint('source IN (\'entrio\', \'croatia\', \'manual\', \'api\', \'facebook\', \'eventbrite\', \'user_generated\')', name='events_source_check'),
        CheckConstraint('approval_status IN (\'pending\', \'approved\', \'rejected\')', name='events_approval_check')
    )