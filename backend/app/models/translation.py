...
class Language(Base):
    ...
    native_name = Column(String(100), nullable=False)  # Hrvatski, English, Deutsch, Italiano
...
class EventTranslation(Base):
    ...
    translation_quality = Column(String(20), default='pending')  # pending, reviewed, approved
...
class StaticContentTranslation(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    key = Column(String(200), nullable=False, index=True)  # 'welcome_message', 'search_placeholder', etc.
    language_code = Column(String(10), ForeignKey('languages.code'), nullable=False)
