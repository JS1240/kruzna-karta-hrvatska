...
from sqlalchemy import Column, Integer, String, DateTime, Date, Boolean, ForeignKey, Text, JSON, Enum as SQLEnum
...
class EventSeries(Base):
    ...
    advance_notice_days = Column(Integer, default=30)  # How far ahead to generate instances
    ...
    recurrence_end_type = Column(SQLEnum(RecurrenceEndType), default=RecurrenceEndType.NEVER)
    ...
    instances = relationship("EventInstance", back_populates="series", cascade="all, delete-orphan")
    modifications = relationship("SeriesModification", back_populates="series", cascade="all, delete-orphan")
...
class EventInstance(Base):
    ...
    # Instance metadata
    instance_status = Column(SQLEnum(EventInstanceStatus), default=EventInstanceStatus.SCHEDULED)
    ...
    modifications = relationship("InstanceModification", back_populates="instance", cascade="all, delete-orphan")
...
class SeriesModification(Base):
    ...
    modification_type = Column(String(50), nullable=False)  # schedule_change, template_update, etc.
...
class InstanceModification(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    instance_id = Column(Integer, ForeignKey("event_instances.id", ondelete="CASCADE"), nullable=False)
    
    # Modification details
    modification_type = Column(String(50), nullable=False)  # reschedule, cancel, content_change
    field_changed = Column(String(100))
    old_value = Column(Text)
    new_value = Column(Text)
...
class SeriesTemplate(Base):
    ...
    # Template content
    template_title_pattern = Column(String(500))  # e.g., "Weekly Yoga Class #{sequence}"
    template_description = Column(Text)
    template_tags = Column(JSON)
    template_metadata = Column(JSON)
