from sqlalchemy import Column, Integer, String, Text, DateTime, Numeric, Boolean, ForeignKey, Table, CheckConstraint, UniqueConstraint
...
class EnhancedVenue(Base):
    ...
    manager = relationship("User", foreign_keys=[manager_id], back_populates="managed_venues")
    facilities = relationship("Facility", secondary=venue_facilities, back_populates="venues")
    availability_slots = relationship("VenueAvailability", back_populates="venue", cascade="all, delete-orphan")
    ...
    pricing_rules = relationship("VenuePricingRule", back_populates="venue", cascade="all, delete-orphan")
    maintenance_logs = relationship("VenueMaintenanceLog", back_populates="venue", cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        CheckConstraint('average_rating >= 0 AND average_rating <= 5', name='venue_rating_check'),
...
class Facility(Base):
    ...
    # Relationships
    venues = relationship("EnhancedVenue", secondary=venue_facilities, back_populates="facilities")
...
class VenueAvailability(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    venue_id = Column(Integer, ForeignKey('enhanced_venues.id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    status = Column(SQLEnum(AvailabilityStatus), default=AvailabilityStatus.AVAILABLE, index=True)
    ...
    __table_args__ = (
        ...
        UniqueConstraint('venue_id', 'start_time', 'end_time', name='venue_availability_unique')
    )
...
class VenueBooking(Base):
    ...
    booking_status = Column(SQLEnum(BookingStatus), default=BookingStatus.PENDING, index=True)
    ...
    payments = relationship("VenuePayment", back_populates="booking", cascade="all, delete-orphan")
...
class VenuePayment(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    booking_id = Column(Integer, ForeignKey('venue_bookings.id', ondelete='CASCADE'), nullable=False, index=True)
...
class VenueReview(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint('cleanliness_rating IS NULL OR (cleanliness_rating >= 1 AND cleanliness_rating <= 5)', name='cleanliness_rating_check'),
        CheckConstraint('staff_rating IS NULL OR (staff_rating >= 1 AND staff_rating <= 5)', name='staff_rating_check'),
        CheckConstraint('facilities_rating IS NULL OR (facilities_rating >= 1 AND facilities_rating <= 5)', name='facilities_rating_check'),
        CheckConstraint('value_rating IS NULL OR (value_rating >= 1 AND value_rating <= 5)', name='value_rating_check'),
        CheckConstraint('location_rating IS NULL OR (location_rating >= 1 AND location_rating <= 5)', name='location_rating_check'),
        UniqueConstraint('venue_id', 'user_id', 'booking_id', name='unique_venue_review')
    )
...
class VenuePricingRule(Base):
    ...
    description = Column(Text)
    rule_type = Column(String(50), nullable=False)  # seasonal, day_of_week, duration, capacity
    
    # Conditions
    conditions = Column(JSONB, nullable=False)  # Flexible condition storage
    
    # Pricing adjustments
    price_adjustment_type = Column(String(20), nullable=False)  # percentage, fixed_amount
