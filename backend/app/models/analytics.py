...
class EventView(Base):
    ...
    )  # 'search', 'category', 'featured', 'direct'
...
class UserInteraction(Base):
    ...
    )  # Interaction value (e.g., share platform)

    # Timestamps
    interacted_at = Column(
...
class EventPerformanceMetrics(Base):
    ...
    __table_args__ = (Index("idx_event_metrics_daily", "event_id", "date"),)
...
Index(
    ...
    UserInteraction.interacted_at,
)
Index(
    "idx_performance_metrics_event_date",
    EventPerformanceMetrics.event_id,
    EventPerformanceMetrics.date,
)
Index(
    "idx_platform_metrics_type_date", PlatformMetrics.metric_type, PlatformMetrics.date
)
