...
class User(Base):
    ...
    # Relationships
    favorite_events = relationship("Event", secondary=user_favorites, back_populates="favorited_by")
    ...
    owned_venues = relationship("EnhancedVenue", foreign_keys="EnhancedVenue.owner_id", back_populates="owner")
    managed_venues = relationship("EnhancedVenue", foreign_keys="EnhancedVenue.manager_id", back_populates="manager")
    ...
    post_reactions = relationship("PostReaction", back_populates="user", cascade="all, delete-orphan")
    ...
    received_connections = relationship("UserConnection", foreign_keys="UserConnection.addressee_id", back_populates="addressee")
    ...
    notifications = relationship("SocialNotification", foreign_keys="SocialNotification.user_id", back_populates="user")
    mentioned_in_posts = relationship("SocialPost", secondary="post_mentions", back_populates="mentions")
    ...
    organized_events = relationship("Event", foreign_keys="Event.organizer_id", back_populates="event_organizer")
...
class UserRole(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(50), unique=True, nullable=False)  # admin, moderator, organizer, user
