...
def system_health_check(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            organizer_role = (
                db.query(UserRole).filter(UserRole.name == "event_organizer").first()
            )
        ...
        try:
            ...
            user_events = (
                db.query(Event).filter(Event.is_user_generated == True).count()
            )
        ...
        results["overall_status"] = (
            "healthy" if not errors else f"issues: {len(errors)}"
        )
...
def get_sample_data(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        category_data = (
            [{"id": c.id, "name": c.name} for c in categories] if categories else []
        )
        ...
        venue_data = (
            [
                {"id": v.id, "name": v.name, "address": getattr(v, "address", "")}
                for v in venues
            ]
            if venues
            else []
...
def create_sample_category(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        existing = (
            db.query(EventCategory).filter(EventCategory.name == "Test Events").first()
        )
        if existing:
            return {
        ...
        category = EventCategory(
            name="Test Events", description="Category for testing user-generated events"
        )
        db.add(category)
        db.commit()
        db.refresh(category)
