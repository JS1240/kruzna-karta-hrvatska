...
def get_social_feed(
    ...
    return SocialFeedResponse(
        posts=posts, total=total, page=page, size=size, pages=pages, has_more=has_more
    )
...
def react_to_comment(
    ...
    """React to comment"""
    service = SocialService(db)
    reaction = service.toggle_comment_reaction(
        comment_id, current_user.id, reaction_data
    )
...
def respond_to_connection_request(
    ...
    service = SocialService(db)
    connection = service.respond_to_connection_request(
        connection_id, current_user.id, response_data
    )
    if not connection:
...
@router.post(
    "/report", response_model=ContentReport, status_code=status.HTTP_201_CREATED
)
def report_content(
...
def get_trending_hashtags(
    ...
    return TrendingHashtagsResponse(hashtags=hashtags, total=len(hashtags))
...
def search_users(
    ...
    return {"users": users, "total": total, "page": page, "size": size, "pages": pages}
...
def get_user_social_stats(user_id: int, db: Session = Depends(get_db)):
    """Get user's social statistics"""
    service = SocialService(db)
