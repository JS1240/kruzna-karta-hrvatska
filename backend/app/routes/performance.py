...
from ..core.database import get_db, health_check_db, get_db_pool_status
...
def invalidate_cache_namespace(
    namespace: str,
    pattern: str = Query("*", description="Pattern to match keys (default: all keys in namespace)"),
    current_user: User = Depends(get_current_active_user)
):
    ...
    # Validate namespace
    valid_namespaces = [
        "events", "event_detail", "categories", "venues", "popular_events",
        "search_results", "analytics", "user_session", "translations", "static_content"
    ]
...
def _generate_pool_recommendations(pool_stats: Dict[str, Any]) -> list:
    ...
    if utilization > 90:
        recommendations.append("Critical: Pool utilization very high. Increase pool size immediately.")
    elif utilization > 80:
        recommendations.append("Warning: Pool utilization high. Consider increasing pool size.")
    elif utilization > 60:
    ...
    if pool_stats.get("invalid", 0) > 0:
        recommendations.append(f"Warning: {pool_stats['invalid']} invalid connections detected.")
