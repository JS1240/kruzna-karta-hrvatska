...
from ..models.venue_management import EnhancedVenue as EnhancedVenueModel
from ..models.venue_management import VenueAvailability as VenueAvailabilityModel
from ..models.venue_management import VenueReview as VenueReviewModel
...
# Venue Endpoints
@router.post(
    "/venues", response_model=EnhancedVenue, status_code=status.HTTP_201_CREATED
)
def create_venue(
...
def update_venue(
    ...
    if venue.owner_id != current_user.id and venue.manager_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to modify this venue"
        )
...
def search_venues(
    ...
    return VenueListResponse(
        venues=venues, total=total, page=page, size=size, pages=pages
    )
...
def get_venue_statistics(
    ...
    if venue.owner_id != current_user.id and venue.manager_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to view venue statistics"
        )
...
def add_venue_facility(
    ...
    if venue.owner_id != current_user.id and venue.manager_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to modify venue facilities"
        )
...
def create_availability_slot(
    ...
    if venue.owner_id != current_user.id and venue.manager_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to manage venue availability"
        )
...
def get_venue_availability(
    ...
    return VenueAvailabilityResponse(
        availability_slots=availability_slots, total=len(availability_slots)
    )
...
def update_availability_slot(
    ...
    service = VenueManagementService(db)
    availability = (
        db.query(VenueAvailabilityModel)
        .filter(VenueAvailabilityModel.id == availability_id)
        .first()
    )
    ...
    if venue.owner_id != current_user.id and venue.manager_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to modify venue availability"
        )

    updated_availability = service.update_availability_slot(
        availability_id, availability_data
    )
...
def create_venue_booking(
    ...
    is_available = service.check_availability(
        venue_id, booking_data.start_datetime, booking_data.end_datetime
    )
    if not is_available:
        raise HTTPException(
            status_code=400, detail="Venue not available for selected time"
        )
...
def update_booking(
    ...
    if (
        ...
        and venue.manager_id != current_user.id
    ):
        raise HTTPException(
...
def search_bookings(
    ...
    if venue_id:
        venue = service.get_venue(venue_id)
        if venue and (
            venue.owner_id != current_user.id and venue.manager_id != current_user.id
        ):
        ...
        user_venues = [
            v.id
            for v in db.query(EnhancedVenueModel)
            .filter(
                or_(
                    EnhancedVenueModel.owner_id == current_user.id,
                    EnhancedVenueModel.manager_id == current_user.id,
                )
            )
            .all()
        ]
        ...
        bookings = [
            b
            for b in bookings
            if b.customer_id == current_user.id or b.venue_id in user_venues
        ]
        total = len(bookings)
    ...
    return VenueBookingResponse(
        bookings=bookings, total=total, page=page, size=size, pages=pages
    )
...
def get_venue_reviews(
    ...
    for i in range(1, 6):
        count = (
            db.query(VenueReviewModel)
            .filter(
                and_(
                    VenueReviewModel.venue_id == venue_id,
                    VenueReviewModel.rating == i,
                    VenueReviewModel.is_public == True,
                )
            )
            .count()
        )
...
def update_review(
    ...
    is_venue_manager = (
        venue.owner_id == current_user.id or venue.manager_id == current_user.id
    )
    ...
    if is_venue_manager and not is_review_author:
        if any(
            field in review_data.dict(exclude_unset=True)
            for field in ["rating", "title", "review_text"]
        ):
            raise HTTPException(
                status_code=403, detail="Venue managers can only add responses"
            )
