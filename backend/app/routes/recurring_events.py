...
def create_event_series(
    ...
    try:
        ...
        result = recurring_service.create_event_series(
            db=db, organizer_id=current_user.id, series_data=series_request.model_dump()
        )
...
def get_event_series(
    ...
    try:
        ...
        series = (
            ...
            .filter(EventSeries.id == series_id)
            .first()
        )
        ...
        return {
            ...
            "category": (
                {"id": series.category.id, "name": series.category.name}
                if series.category
                else None
            ),
            "venue": (
            ...
            "recurrence": {
                ...
                "end_date": (
                    series.recurrence_end_date.isoformat()
            ...
            "last_generated_at": (
                series.last_generated_at.isoformat()
                if series.last_generated_at
                else None
            ),
        }
...
def modify_event_instance(
    ...
    try:
        ...
        instance = (
            ...
            .join(EventSeries)
            .filter(EventInstance.id == instance_id)
            .first()
        )
...
def cancel_event_instance(
    ...
    try:
        ...
        result = recurring_service.cancel_instance(
            db=db, instance_id=instance_id, user_id=current_user.id, reason=reason
        )
...
def get_my_event_series(
    ...
    try:
        ...
        query = (
            db.query(EventSeries)
            .options(joinedload(EventSeries.category), joinedload(EventSeries.venue))
        ...
        series_list = (
            query.order_by(EventSeries.created_at.desc()).offset(skip).limit(size).all()
        )
            ...
            formatted_series.append(
                {
                    ...
                    "venue": (
                        {"id": series.venue.id, "name": series.venue.name}
                        if series.venue
                        else None
                    ),
                    "statistics": {
...
def get_upcoming_instances(
    ...
    try:
        ...
        instances = recurring_service.get_upcoming_instances(
            db=db, user_id=current_user.id, days_ahead=days_ahead, limit=limit
        )
...
def get_series_statistics(
    ...
    try:
        ...
        statistics = recurring_service.generate_series_statistics(
            db=db, series_id=series_id
        )
