...
from ..core.security import get_gdpr_service, get_security_service, DataSubjectRequest
    ...
    try:
        ...
        if request_data.request_type in ["access", "portability"]:
            background_tasks.add_task(
                gdpr_service.process_data_subject_request,
                data_request
            )
            ...
            if result["status"] == "completed":
                message = f"Data {request_data.request_type} request completed successfully."
            else:
...
async def get_privacy_settings(current_user: User = Depends(get_current_user)) -> PrivacySettings:
