    ...
    try:
        ...
        if result.get("event_type") == "payment_succeeded":
            await handle_payment_success(
                db, result["booking_reference"], result["payment_intent_id"]
            )
            ...
            await handle_dispute_created(db, result["charge_id"], result["dispute_id"])
...
async def handle_payment_success(
    db: Session, booking_reference: str, payment_intent_id: str
):
    ...
    try:
        ...
        if booking.booking_status != BookingStatus.PAID:
            ...
            existing_tickets = (
                db.query(Ticket).filter(Ticket.booking_id == booking.id).count()
            )

            if existing_tickets == 0:
                tickets = booking_service._generate_tickets(db, booking)
                logger.info(
                    f"Generated {len(tickets)} tickets for booking {booking_reference}"
                )
...
async def handle_dispute_created(db: Session, charge_id: str, dispute_id: str):
    """Handle chargeback/dispute webhook."""
    try:
        # Find payment by charge ID
        payment = (
            db.query(Payment).filter(Payment.external_payment_id == charge_id).first()
        )
