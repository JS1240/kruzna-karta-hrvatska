...
from ..models.translation_schemas import (
    Language as LanguageSchema, LanguageCreate, LanguageUpdate, LanguageResponse,
    EventTranslationCreate, EventTranslationUpdate, EventTranslation,
    CategoryTranslationCreate, CategoryTranslationUpdate, CategoryTranslation,
    VenueTranslationCreate, VenueTranslationUpdate, VenueTranslation,
    StaticContentTranslationCreate, StaticContentTranslationUpdate, StaticContentTranslation,
    BulkTranslationRequest, BulkTranslationResponse,
    TranslationStatsResponse, TranslationStatusResponse
)
...
def get_languages(db: Session = Depends(get_db)):
    ...
    languages = db.query(Language).filter(Language.is_active == True).order_by(Language.is_default.desc(), Language.name.asc()).all()
...
@router.put("/events/{event_id}/translations/{language_code}", response_model=EventTranslation)
def update_event_translation(
    ...
    if 'translation_quality' in update_data and update_data['translation_quality'] == 'reviewed':
...
def get_category_translations(
    ...
    for language in languages:
        translation = translation_service.get_category_translation(category_id, language.code)
        if translation:
            translations[language.code] = {
                "name": translation.name,
                "description": translation.description,
...
@router.post("/categories/{category_id}/translations", response_model=CategoryTranslation)
...
def get_translation_stats(
    ...
    from ..models.translation import EventTranslation, CategoryTranslation, VenueTranslation, StaticContentTranslation
    from ..models.event import Event
    from ..models.category import EventCategory
    from ..models.venue import Venue
    
    # Count translations
    events_translated = db.query(EventTranslation).filter(EventTranslation.language_code == language_code).count()
    categories_translated = db.query(CategoryTranslation).filter(CategoryTranslation.language_code == language_code).count()
    venues_translated = db.query(VenueTranslation).filter(VenueTranslation.language_code == language_code).count()
    static_content_translated = db.query(StaticContentTranslation).filter(StaticContentTranslation.language_code == language_code).count()
    ...
    total_translated = events_translated + categories_translated + venues_translated + static_content_translated
    total_entities = total_events + total_categories + total_venues
    
    completion_percentage = (total_translated / total_entities * 100) if total_entities > 0 else 0
