...
from ..core.monitoring import PrometheusMonitoring, get_monitoring_service
...
async def get_system_metrics(
    ...
    try:
        ...
        return {
            "timestamp": datetime.now().isoformat(),
            "system_metrics": {
                "cpu": {"usage_percent": sys_metrics.get("cpu_usage_percent", 0)},
                "memory": {
                    "usage_bytes": sys_metrics.get("memory_usage_bytes", 0),
                    "usage_mb": round(
                        sys_metrics.get("memory_usage_bytes", 0) / 1024 / 1024, 2
                    ),
                    "usage_percent": sys_metrics.get("memory_usage_percent", 0),
                },
                "disk": {"usage_by_mount": sys_metrics.get("disk_usage", {})},
                "network": {
...
async def acknowledge_alert(
    alert_id: str, current_user: User = Depends(get_current_superuser)
) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "summary": {
                "database": {
                    "status": (
                        "healthy"
                        if health_status["database"]["cache_hit_ratio"] > 90
                        else "warning"
                    ),
                    "active_connections": health_status["database"][
                        "active_connections"
                    ],
                ...
                "system": {
                    ...
                    "status": (
                        "healthy"
                        if health_status["system"]["cpu_usage_percent"] < 80
                        else "warning"
                    ),
                },
                "alerts": {
    ...
    try:
        ...
        return {
            ...
            "alerts": {
                ...
                "enabled_alerts": len(
                    [a for a in monitoring_service.alert_configs if a.enabled]
                ),
                "alert_configs": [
                    {
...
async def toggle_alert(
    metric_name: str, enabled: bool, current_user: User = Depends(get_current_superuser)
) -> Dict[str, Any]:
