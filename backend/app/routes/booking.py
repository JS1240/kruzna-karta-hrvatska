...
def get_booking(
    ...
    try:
        ...
        result = booking_service.get_booking(
            db=db, booking_reference=booking_reference, user_id=current_user.id
        )
...
def get_user_bookings(
    ...
    try:
        ...
        query = (
            db.query(Booking)
            .options(joinedload(Booking.event), joinedload(Booking.ticket_type))
        ...
        bookings = (
            query.order_by(Booking.booking_date.desc()).offset(skip).limit(size).all()
        )
    ...
    try:
        ...
        return {
            "ticket": {
                ...
                "valid_from": (
                    ticket.valid_from.isoformat() if ticket.valid_from else None
                ),
                "valid_until": (
                    ticket.valid_until.isoformat() if ticket.valid_until else None
                ),
                "check_in_time": (
            ...
            "event": (
                {
                    ...
                    "date": (
                        ticket.booking.event.date.isoformat()
                        if ticket.booking.event.date
                        else None
                    ),
                    "location": ticket.booking.event.location,
                }
                if ticket.booking and ticket.booking.event
                else None
            ),
            "ticket_type": (
                {
            ...
            ticket_list.append(
                {
                    ...
                    "sale_start": (
                        ticket_type.sale_start.isoformat()
                        if ticket_type.sale_start
                        else None
                    ),
                    "sale_end": (
                        ticket_type.sale_end.isoformat()
                        if ticket_type.sale_end
                        else None
                    ),
