...
def get_platform_overview(
    ...
    metrics = db.query(PlatformMetrics).filter(
...
def get_real_time_stats(
    ...
    # Today's stats
    todays_views = db.query(EventView).filter(
        func.date(EventView.viewed_at) == today
    ).count()
    
    todays_searches = db.query(SearchLog).filter(
        func.date(SearchLog.searched_at) == today
    ).count()
    
    todays_interactions = db.query(UserInteraction).filter(
        func.date(UserInteraction.interacted_at) == today
    ).count()
    
    # Active users (last hour)
    hour_ago = datetime.now() - timedelta(hours=1)
    active_users_last_hour = db.query(EventView.user_id).filter(
        EventView.viewed_at >= hour_ago,
