...
async def get_croatian_holidays(
    year: int = Query(default=None, description="Year to get holidays for (defaults to current year)"),
    holiday_type: Optional[str] = Query(default=None, description="Filter by holiday type: national, religious, cultural, regional, international"),
    upcoming_only: bool = Query(default=False, description="Only return upcoming holidays"),
    days_ahead: int = Query(default=30, description="Number of days ahead to look for upcoming holidays")
) -> Dict[str, Any]:
    ...
    try:
        ...
        for holiday in holidays:
            holiday_list.append({
                ...
                "regions": [r.value for r in holiday.regions] if holiday.regions else None
            })
...
async def get_region_info(region_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        seasonal_recommendations = holiday_service.get_seasonal_events_recommendation(region, current_month)
...
async def convert_currency(
    ...
    try:
        ...
        # Validate currencies
        supported_codes = [c["code"] for c in currency_service.get_supported_currencies()]
        ...
        return {
            ...
            "exchange_rate": rate,
            "formatted_original": currency_service.format_croatian_price(amount, from_currency),
            "formatted_converted": currency_service.format_croatian_price(converted_amount, to_currency),
...
async def get_event_timing_suggestions(
    event_type: str = Query(..., description="Type of event (e.g., 'outdoor festival', 'cultural event')"),
    region_code: Optional[str] = Query(default=None, description="Croatian region code")
) -> Dict[str, Any]:
    ...
    try:
        return {
            ...
            "tourism_info": {
                ...
                "major_destinations": [
                    "Zagreb", "Split", "Dubrovnik", "Pula", "Rovinj", "Zadar"
                ]
            },
            "event_traditions": {
