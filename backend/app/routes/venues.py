...
from ..models.schemas import Venue as VenueSchema
...
def search_venues(params: VenueSearchParams = Depends(), db: Session = Depends(get_db)):
...
def create_venue(venue: VenueCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Venue with this name already exists in this city"
        )
