...
def update_event_approval(
    ...
    try:
        ...
        logger.info(f"Admin {current_user.id} {approval_data.approval_status} event {event.id}")
...
def get_organizer_stats(
    ...
    try:
        ...
        ).join(Event).filter(Event.organizer_id == current_user.id).first()
...
def get_event_performance(
    ...
    try:
        ...
        event_performance = db.query(
            ...
            Event.organizer_id == current_user.id
        ).group_by(Event.id).order_by(func.sum(Booking.total_price).desc().nullslast()).all()
            ...
            "events": [
                {
                    ...
                    "conversion_rate": round(
                        (event.total_bookings or 0) / max(event.view_count or 1, 1) * 100, 2
                    )
                }
...
def get_ticket_type_analytics(
    ...
    try:
        ...
        ).join(Booking).join(Event).filter(
            Event.organizer_id == current_user.id
        ).group_by(TicketType.id).order_by(func.sum(Booking.total_price).desc()).all()
            ...
            "ticket_types": [
                {
                    ...
                    "sold_percentage": round(
                        (ticket.tickets_sold or 0) / max(ticket.total_quantity, 1) * 100, 2
                    )
                }
...
def get_booking_conversion_metrics(
    ...
    try:
        ...
        total_views = db.query(func.sum(Event.view_count)).filter(
            Event.organizer_id == current_user.id
        ).scalar() or 0
        
        total_bookings = db.query(func.count(Booking.id)).join(Event).filter(
            Event.organizer_id == current_user.id
        ).scalar() or 0
        
        # Get conversion by event status
        conversions_by_status = db.query(
            Event.event_status,
            func.sum(Event.view_count).label('total_views'),
            func.count(Booking.id).label('total_bookings')
        ).outerjoin(Booking).filter(
            Event.organizer_id == current_user.id
        ).group_by(Event.event_status).all()
        
        return {
            "overall_conversion_rate": round(total_bookings / max(total_views, 1) * 100, 2),
            ...
            "conversion_by_status": [
                {
                    ...
                    "conversion_rate": round(
                        (conv.total_bookings or 0) / max(conv.total_views or 1, 1) * 100, 2
                    )
                }
...
def get_geographic_revenue(
    ...
    try:
        ...
        geographic_revenue = db.query(
            ...
            Event.organizer_id == current_user.id
        ).group_by(Event.location).order_by(func.sum(Booking.total_price).desc().nullslast()).all()
            ...
            "locations": [
                {
                    ...
                    "avg_revenue_per_event": round(
                        float(location.total_revenue or 0) / max(location.event_count, 1), 2
                    )
                }
