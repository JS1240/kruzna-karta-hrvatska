...
class AnalyticsTaskScheduler:
    ...
    def aggregate_weekly_metrics(self, target_date: Optional[date] = None):
        ...
        logger.info(
            f"Starting weekly metrics aggregation for week ending {target_date}"
        )
        ...
        try:
            ...
            logger.error(
                f"Error aggregating weekly metrics for {target_date}: {str(e)}"
            )
            raise
    ...
    def aggregate_monthly_metrics(self, target_date: Optional[date] = None):
        ...
        logger.info(
            f"Starting monthly metrics aggregation for month ending {target_date}"
        )
        ...
        try:
            ...
            logger.error(
                f"Error aggregating monthly metrics for {target_date}: {str(e)}"
            )
            raise
    ...
    def cleanup_old_raw_data(self, retention_days: int = 90):
        ...
        try:
            with self.get_db_session() as db:
                ...
                old_views = (
                    db.query(EventView)
                    .filter(EventView.viewed_at < cutoff_date)
                    .count()
                )
                old_searches = (
                    db.query(SearchLog)
                    .filter(SearchLog.searched_at < cutoff_date)
                    .count()
                )
                old_interactions = (
                    db.query(UserInteraction)
                    .filter(UserInteraction.interacted_at < cutoff_date)
                    .count()
                )

                logger.info(
                    f"Found {old_views} old event views, {old_searches} old searches, {old_interactions} old interactions"
                )
                ...
                db.query(UserInteraction).filter(
                    UserInteraction.interacted_at < cutoff_date
                ).delete()

                db.commit()
                logger.info(
                    f"Cleanup completed: removed {old_views + old_searches + old_interactions} old records"
                )
        ...
        try:
            with self.get_db_session() as db:
                ...
                popular_events = analytics.get_popular_events(
                    date_from=target_date, date_to=target_date, limit=5
                )
                ...
                report = {
                    ...
                    "platform_metrics": {
                        ...
                        "search_success_rate": float(
                            platform_metrics.search_success_rate
                        ),
                    },
            ...
            logger.error(
                f"Error generating analytics report for {target_date}: {str(e)}"
            )
            raise
        ...
        try:
            with self.get_db_session() as db:
                ...
                for threshold in active_thresholds:
                    ...
                    if threshold.threshold_type == "min" and current_value < float(
                        threshold.threshold_value
                    ):
                        threshold_violated = True
                    elif threshold.threshold_type == "max" and current_value > float(
                        threshold.threshold_value
                    ):
                        threshold_violated = True
                    ...
                    if threshold_violated:
                        ...
                        if not existing_alert:
                            ...
                            violation_percentage = (
                                abs(current_value - float(threshold.threshold_value))
                            ...
                            logger.warning(
                                f"Alert triggered: {alert_message} (Severity: {severity})"
                            )

                db.commit()
                logger.info(
                    f"Metric alert check completed. {alerts_triggered} new alerts triggered."
                )
