...
class InfoZagrebScraper(BaseScraper):
    ...
    def transform_to_event(self, raw_data: Dict[str, Any]) -> Optional[EventCreate]:
        ...
        try:
            ...
            location = self.clean_text(raw_data.get("location", "Zagreb"))
...
async def scrape_infozagreb_events(max_pages: int = 10, use_fallbacks: bool = True) -> Dict:
    """Enhanced InfoZagreb scraping with multiple strategies.
    
    Args:
        max_pages: Maximum number of pages to scrape
        use_fallbacks: Whether to use multi-strategy fallback approach
    ...
    try:
        if use_fallbacks:
            logger.info("Using enhanced multi-strategy scraping")
            events = await scraper.scrape_with_fallbacks(max_pages=max_pages)
        else:
            logger.info("Using standard base scraper method")
            events = await scraper.scrape_all_events(max_pages=max_pages)
        
        if not events:
            logger.warning("No events found with any scraping strategy")
            return {
                "status": "warning",
                "scraped_events": 0,
                "saved_events": 0,
                "message": "No events found from InfoZagreb.hr - site may have changed structure",
            }
        ...
        return {
            ...
            "saved_events": saved,
            "message": f"Successfully scraped {len(events)} events from InfoZagreb.hr, saved {saved} new events",
            "strategies_used": "multi-strategy fallback" if use_fallbacks else "standard",
        }
        
    except Exception as e:
        logger.error(f"InfoZagreb scraping failed: {e}")
        return {
            "status": "error", 
            "message": f"InfoZagreb scraping failed: {e}",
            "scraped_events": 0,
            "saved_events": 0,
        }
