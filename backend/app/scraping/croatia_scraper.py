    ...
    def transform_to_event_create(scraped_data: Dict) -> Optional[EventCreate]:
        ...
        try:
            ...
            description = CroatiaEventDataTransformer.clean_text(
                ...
                scraped_data.get('shortDescription', '') or
                scraped_data.get('intro', '')
            )
            ...
            # Parse dates
            start_date_str = scraped_data.get('startDate', '') or scraped_data.get('date', '')
            parsed_date = CroatiaEventDataTransformer.parse_croatian_date(start_date_str)
            if not parsed_date:
            ...
            # Handle image URL
            image_url = scraped_data.get('image', '') or scraped_data.get('imageUrl', '')
            if image_url and not image_url.startswith('http'):
                image_url = urljoin(BASE_URL, image_url)
            
            # Handle event link
            link = scraped_data.get('link', '') or scraped_data.get('url', '') or scraped_data.get('sefUrl', '')
...
class CroatiaPlaywrightScraper:
    ...
    async def scrape_with_playwright(self, start_url: str = EVENTS_URL, max_pages: int = 5) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await page.wait_for_selector('[class*="event"], [class*="card"], [class*="item"], a[href*="/dogadanja/"]', timeout=30000)
...
async def scrape_croatia_events(max_pages: int = 5) -> Dict:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Croatia.hr scraping failed: {str(e)}"
        }