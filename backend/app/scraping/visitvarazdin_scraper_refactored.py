"""Refactored VisitVarazdin.hr events scraper using BaseScraper."""
...
class VisitVarazdinScraper(BaseScraper):
    """Scraper for VisitVarazdin.hr events using the base scraper infrastructure."""

    def __init__(self):
        super().__init__(
            base_url="https://visitvarazdin.hr",
            events_url="https://visitvarazdin.hr/en/events",
            source_name="visitvarazdin"
        )
        ...
        for pattern in patterns:
            ...
            if m:
                try:
                    ...
                    else:  # DD.MM.YYYY or DD Month YYYY
                        day, month, year = m.groups()
                        if month.isdigit():
                            return date(int(year), int(month), int(day))
                        # Month name
                        month_num = CROATIAN_MONTHS.get(month.lower())
                        if month_num:
    ...
    def transform_to_event(self, raw_data: Dict[str, str]) -> Optional[EventCreate]:
        ...
        try:
            ...
            location = self.clean_text(raw_data.get("location", "Varazdin"))
...
async def scrape_visitvarazdin_events(max_pages: int = 5) -> Dict:
    """Main scraping function for VisitVarazdin events."""
