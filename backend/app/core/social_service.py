...
from ..models.social import (
    ...
    ContentReport, HashtagTrend, SocialGroup, GroupMembership, GroupPost,
    ConnectionStatus, PostType, ReactionType, NotificationType, ReportReason,
    post_mentions
)
...
from ..models.social_schemas import (
    UserSocialProfileCreate, UserSocialProfileUpdate, SocialFeedParams,
    SocialPostCreate, SocialPostUpdate, PostCommentCreate, PostCommentUpdate,
    PostReactionCreate, EventReviewCreate, EventReviewUpdate,
    UserConnectionCreate, UserConnectionUpdate, UserFollowCreate,
    EventAttendanceCreate, EventAttendanceUpdate, SocialNotificationCreate,
    ContentReportCreate, UserSearchParams, PostSearchParams
)
...
class SocialService:
    ...
    def get_or_create_user_profile(self, user_id: int) -> UserSocialProfile:
        ...
        profile = self.db.query(UserSocialProfile).filter(UserSocialProfile.user_id == user_id).first()
        if not profile:
    ...
    def update_user_profile(self, user_id: int, profile_data: UserSocialProfileUpdate) -> UserSocialProfile:
    ...
    def get_user_profile(self, user_id: int) -> Optional[UserSocialProfile]:
        ...
        return self.db.query(UserSocialProfile).filter(UserSocialProfile.user_id == user_id).first()
    ...
    def update_post(self, post_id: int, user_id: int, post_data: SocialPostUpdate) -> Optional[SocialPost]:
    ...
    def get_social_feed(self, user_id: int, feed_params: SocialFeedParams) -> Tuple[List[SocialPost], int]:
        ...
        if feed_params.feed_type == "timeline":
            ...
            connection_ids = [c.addressee_id if c.requester_id == user_id else c.requester_id for c in connections]
    ...
    def search_posts(self, search_params: PostSearchParams) -> Tuple[List[SocialPost], int]:
    ...
    # Comments Management
    def create_comment(self, post_id: int, user_id: int, comment_data: PostCommentCreate) -> PostComment:
        """Create post comment"""
        post = self.get_post(post_id)
    ...
    def update_comment(self, comment_id: int, user_id: int, comment_data: PostCommentUpdate) -> Optional[PostComment]:
    ...
    def delete_comment(self, comment_id: int, user_id: int) -> bool:
        ...
        if comment.parent_comment_id:
            parent_comment = self.db.query(PostComment).filter(
                PostComment.id == comment.parent_comment_id
            ).first()
            if parent_comment:
                parent_comment.replies_count -= 1
    ...
    def toggle_post_reaction(self, post_id: int, user_id: int, reaction_data: PostReactionCreate) -> Optional[PostReaction]:
    ...
    def toggle_comment_reaction(self, comment_id: int, user_id: int, reaction_data: PostReactionCreate) -> Optional[CommentReaction]:
    ...
    def create_event_review(self, user_id: int, review_data: EventReviewCreate) -> EventReview:
        """Create event review"""
        # Check if user already reviewed this event
        existing_review = self.db.query(EventReview).filter(
            and_(
        ...
        logger.info(f"Created event review {review.id} for event {review_data.event_id}")
        return review
    
    def update_event_review(self, review_id: int, user_id: int, review_data: EventReviewUpdate) -> Optional[EventReview]:
        """Update event review"""
    ...
    def get_event_reviews(self, event_id: int, limit: int = 20, offset: int = 0) -> Tuple[List[EventReview], int]:
        """Get reviews for an event"""
        query = self.db.query(EventReview).filter(
            and_(
                EventReview.event_id == event_id,
                EventReview.is_public == True
            )
        )
        
        total = query.count()
        reviews = query.order_by(desc(EventReview.created_at)).offset(offset).limit(limit).all()
        
        return reviews, total
    
    # User Connections Management
    def send_connection_request(self, requester_id: int, connection_data: UserConnectionCreate) -> UserConnection:
        ...
        logger.info(f"Connection request sent from {requester_id} to {connection_data.addressee_id}")
        return connection
    
    def respond_to_connection_request(self, connection_id: int, user_id: int, response_data: UserConnectionUpdate) -> Optional[UserConnection]:
        """Respond to connection request"""
        connection = self.db.query(UserConnection).filter(UserConnection.id == connection_id).first()
    ...
    def get_user_connections(self, user_id: int, status: Optional[ConnectionStatus] = None) -> List[UserConnection]:
    ...
    # Follow System
    def follow_user(self, follower_id: int, follow_data: UserFollowCreate) -> UserFollow:
        """Follow a user"""
    ...
    # Event Attendance
    def update_event_attendance(self, user_id: int, attendance_data: EventAttendanceCreate) -> EventAttendance:
        """Update event attendance status"""
        # Check if attendance record exists
        existing = self.db.query(EventAttendance).filter(
            and_(
    ...
    def check_in_to_event(self, user_id: int, event_id: int, location: Optional[str] = None) -> bool:
        """Check in to event"""
        attendance = self.db.query(EventAttendance).filter(
            and_(
    ...
    # Notifications
    def get_user_notifications(self, user_id: int, limit: int = 20, offset: int = 0) -> Tuple[List[SocialNotification], int, int]:
        """Get user notifications"""
        query = self.db.query(SocialNotification).filter(SocialNotification.user_id == user_id)
        
        total = query.count()
        unread_count = query.filter(SocialNotification.is_read == False).count()
        
        notifications = query.order_by(desc(SocialNotification.created_at)).offset(offset).limit(limit).all()
        
        return notifications, total, unread_count
    
    def mark_notification_as_read(self, notification_id: int, user_id: int) -> bool:
        """Mark notification as read"""
        notification = self.db.query(SocialNotification).filter(
            and_(
    ...
    def mark_all_notifications_as_read(self, user_id: int) -> int:
        ...
        count = self.db.query(SocialNotification).filter(
            and_(
                SocialNotification.user_id == user_id,
    ...
    # Content Reporting
    def report_content(self, reporter_id: int, report_data: ContentReportCreate) -> ContentReport:
    ...
    def get_trending_hashtags(self, limit: int = 10) -> List[HashtagTrend]:
        ...
        return self.db.query(HashtagTrend).filter(
            HashtagTrend.is_trending == True
        ).order_by(desc(HashtagTrend.trend_score)).limit(limit).all()
    ...
    def search_users(self, search_params: UserSearchParams) -> Tuple[List[User], int]:
        ...
        query = self.db.query(User).join(UserProfile, User.id == UserProfile.user_id, isouter=True)
        ...
        if search_params.location:
            query = query.filter(UserProfile.location.ilike(f"%{search_params.location}%"))
        ...
        if search_params.is_event_organizer is not None:
            query = query.filter(UserProfile.is_event_organizer == search_params.is_event_organizer)
        
        if search_params.is_venue_owner is not None:
            query = query.filter(UserProfile.is_venue_owner == search_params.is_venue_owner)
    ...
    def _update_hashtag_trends(self, hashtags: List[str]):
        ...
        for hashtag in hashtags:
            trend = self.db.query(HashtagTrend).filter(HashtagTrend.hashtag == hashtag).first()
            if trend:
                trend.usage_count += 1
                trend.daily_usage += 1
    ...
    def _create_notification(self, user_id: int, notification_type: NotificationType, title: str, **kwargs):
