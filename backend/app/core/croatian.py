...
class CroatianHolidayService:
    ...
    def get_croatian_holidays(self, year: int) -> List[CroatianHoliday]:
        ...
        if cache_key in self.holidays_cache and datetime.now() < self.cache_expiry:
    ...
    def _get_cultural_holidays(self, year: int) -> List[CroatianHoliday]:
        ...
        celebrations = [
            (
                ...
                3,
                8,
                CroatianHolidayType.INTERNATIONAL,
            ),
            (
                "Dan planeta Zemlje",
                "Earth Day",
            ...
            (
                "Dan hrvatskog filmskog stvaralaštva",
                "Croatian Film Day",
                12,
                21,
                CroatianHolidayType.CULTURAL,
    ...
    def get_seasonal_events_recommendation(
        self, region: CroatianRegion, month: int
    ) -> List[str]:
        ...
        if 6 <= month <= 9:
            if region in [
                CroatianRegion.SPLIT_DALMATIA,
                CroatianRegion.ISTRIA,
                CroatianRegion.DUBROVNIK_NERETVA,
            ]:
                return [
                    "Outdoor concerts",
                    "Beach festivals",
    ...
    def get_supported_currencies(self) -> List[Dict[str, str]]:
        ...
        return [
            {"code": "EUR", "name": "Euro", "name_hr": "Euro", "symbol": "€"},
            {
                "code": "USD",
                "name": "US Dollar",
                "name_hr": "Američki dolar",
                "symbol": "$",
            },
            {
                "code": "GBP",
                "name": "British Pound",
                "name_hr": "Britanska funta",
                "symbol": "£",
            },
            {
                "code": "CHF",
                "name": "Swiss Franc",
                "name_hr": "Švicarski franak",
                "symbol": "CHF",
            },
        ]
...
class CroatianEventsService:
    ...
    def enrich_event_with_croatian_context(
        ...
        if event_date:
            ...
            region = (
                self.holiday_service.get_region_by_city(location.split(",")[0].strip())
                if location
                else None
            )
            ...
            if region:
                seasonal_recommendations = (
                    self.holiday_service.get_seasonal_events_recommendation(
                        region, month
                    )
                )
    ...
    def suggest_croatian_event_timing(
        self, event_type: str, region: Optional[CroatianRegion] = None
    ) -> Dict[str, Any]:
