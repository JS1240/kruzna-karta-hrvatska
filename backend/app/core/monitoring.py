...
class PrometheusMonitoring:
    ...
    async def collect_database_metrics(self, force_refresh: bool = False) -> DatabaseMetrics:
        ...
        try:
            ...
            connection_stats = db.execute(text("""
                ...
                GROUP BY state
            """), {"db_name": settings.db_name}).fetchall()
            ...
            """), {"db_name": settings.db_name}).fetchone()
            ...
            total_queries = query_stats.total_queries if query_stats and query_stats.total_queries else 0
            slow_queries = query_stats.slow_queries if query_stats and query_stats.slow_queries else 0
            ...
            bgwriter_stats = db.execute(text("""
                SELECT 
                    checkpoints_timed,
            ...
            checkpoints_timed = bgwriter_stats.checkpoints_timed if bgwriter_stats else 0
            checkpoints_requested = bgwriter_stats.checkpoints_requested if bgwriter_stats else 0
            buffers_checkpoint = bgwriter_stats.buffers_checkpoint if bgwriter_stats else 0
            ...
            temp_files = temp_stats.temp_files if temp_stats and temp_stats.temp_files else 0
            ...
            return DatabaseMetrics(
                active_connections=0, idle_connections=0, total_connections=0,
                database_size_bytes=0, total_queries=0, slow_queries=0,
                cache_hit_ratio=0.0, deadlocks=0, locks_waiting=0,
                temp_files=0, temp_bytes=0, checkpoints_timed=0,
    ...
    async def collect_application_metrics(self) -> ApplicationMetrics:
        ...
        try:
            ...
            active_events = sum(stat.count for stat in event_stats if stat.event_status == 'active')
            ...
            # Scraping statistics
            scraping_stats = db.execute(text("""
                SELECT 
            ...
            scraping_events_total = scraping_stats.total_scraped if scraping_stats else 0
            scraping_success_rate = 95.0  # Placeholder - would calculate from scraping logs
            ...
            return ApplicationMetrics(
                total_events=0, active_events=0, total_users=0, active_users=0,
                api_requests_total=0, api_requests_per_minute=0.0,
                cache_hit_ratio=0.0, cache_miss_ratio=0.0,
                scraping_events_total=0, scraping_success_rate=0.0
            )
        ...
        try:
            ...
            for alert_config in self.alert_configs:
                ...
                if alert_config.comparison == "gt" and metric_value > alert_config.threshold:
                    ...
                    threshold_exceeded = True
                elif alert_config.comparison == "gte" and metric_value >= alert_config.threshold:
                    threshold_exceeded = True
                elif alert_config.comparison == "lte" and metric_value <= alert_config.threshold:
                    threshold_exceeded = True
...
def monitor_api_request(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            monitoring_service.record_api_request(method, endpoint, status_code, duration)
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                monitoring_service.record_cache_operation(operation, "hit" if result else "miss")
                return result
            except Exception as e:
