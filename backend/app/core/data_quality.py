...
class DataQualityValidator:
    ...
    CROATIAN_CITIES = {
        ...
        'pula', 'karlovac', 'sisak', 'šibenik', 'varaždin', 'dubrovnik',
        'bjelovar', 'vinkovci', 'vukovar', 'velika gorica', 'koprivnica',
        'požega', 'čakovec', 'virovitica', 'đakovo', 'gospić', 'imotski',
        'makarska', 'trogir', 'korčula', 'hvar', 'brač', 'vis', 'lastovo',
        'krk', 'cres', 'lošinj', 'rab', 'pag', 'ugljan', 'dugi otok'
    }
    
    # Common spam/low-quality indicators
    SPAM_INDICATORS = [
        'click here', 'call now', 'limited time', 'act now', 'free money',
        'guaranteed', 'no risk', 'special promotion', 'urgent', 'winner',
    ...
    def validate_name(cls, name: str) -> Tuple[bool, List[str]]:
        ...
        if len(name) < cls.MIN_NAME_LENGTH:
            issues.append(f"Event name too short (minimum {cls.MIN_NAME_LENGTH} characters)")
        
        if len(name) > cls.MAX_NAME_LENGTH:
            issues.append(f"Event name too long (maximum {cls.MAX_NAME_LENGTH} characters)")
    ...
    def validate_description(cls, description: str) -> Tuple[bool, List[str]]:
        ...
        if len(description) < cls.MIN_DESCRIPTION_LENGTH:
            issues.append(f"Description too short (minimum {cls.MIN_DESCRIPTION_LENGTH} characters)")
        
        if len(description) > cls.MAX_DESCRIPTION_LENGTH:
            issues.append(f"Description too long (maximum {cls.MAX_DESCRIPTION_LENGTH} characters)")
        
        # Content validation
        placeholder_texts = [
            'lorem ipsum', 'placeholder', 'sample text', 'test description',
            'description here', 'add description', 'no description'
        ]
        ...
        if len(words) > 10:
            ...
            for word, count in word_count.items():
                if count > len(words) * 0.2:
                    issues.append(f"Description has excessive repetition of word: '{word}'")
    ...
    def validate_location(cls, location: str) -> Tuple[bool, List[str]]:
        ...
        if len(location) < cls.MIN_LOCATION_LENGTH:
            issues.append(f"Location too short (minimum {cls.MIN_LOCATION_LENGTH} characters)")
        
        if len(location) > cls.MAX_LOCATION_LENGTH:
            issues.append(f"Location too long (maximum {cls.MAX_LOCATION_LENGTH} characters)")
    ...
    def validate_event(cls, event: EventCreate) -> Dict[str, Any]:
        ...
        loc_valid, loc_issues = cls.validate_location(event.location)
        date_valid, date_issues = cls.validate_date(event.date)
        url_valid, url_issues = cls.validate_url(event.link) if event.link else (True, [])
        ...
        critical_issues = []
        if not name_valid:
            critical_issues.extend([issue for issue in name_issues if "empty" in issue or "missing" in issue])
        if not date_valid:
        ...
        if len(all_issues) > 0 and validation_result["is_valid"]:
            validation_result["warnings"].append("Event has quality issues but passes validation")
...
class DuplicateDetector:
    ...
    @classmethod
    def calculate_event_similarity(cls, event1: EventCreate, event2: EventCreate) -> Dict[str, float]:
        """Calculate comprehensive similarity between two events."""
        similarity = {
            "name": cls.calculate_text_similarity(event1.name, event2.name),
            "description": cls.calculate_text_similarity(event1.description, event2.description),
    ...
    @classmethod
    def is_duplicate(cls, event1: EventCreate, event2: EventCreate) -> Tuple[bool, Dict[str, Any]]:
        ...
        if (similarity["name"] >= cls.NAME_SIMILARITY_THRESHOLD and 
            similarity["date"] >= 0.9):
            is_duplicate = True
            reason = "High name similarity with same date"
        
        # Rule 2: Exact name match + similar date
        elif (similarity["name"] >= 0.95 and 
              similarity["date"] >= 0.7):
            is_duplicate = True
        ...
        elif (similarity["location"] >= cls.LOCATION_SIMILARITY_THRESHOLD and
              similarity["date"] >= 0.9 and
              similarity["name"] >= 0.7):
            is_duplicate = True
    ...
    @classmethod
    def find_duplicates_in_batch(cls, events: List[EventCreate]) -> List[Dict[str, Any]]:
    ...
    @classmethod
    def find_duplicates_in_database(cls, new_event: EventCreate, db: Session, 
                                   days_window: int = 30) -> List[Tuple[Event, Dict[str, Any]]]:
...
class DataQualityService:
    ...
    def process_scraped_events(self, events: List[EventCreate], 
        ...
        if remove_duplicates and len(results["valid_events"]) > 1:
            ...
            batch_duplicates = self.duplicate_detector.find_duplicates_in_batch(valid_events_only)
            ...
            if batch_duplicates:
                ...
                results["valid_events"] = [
                    item for i, item in enumerate(results["valid_events"])
        ...
        for item in results["valid_events"]:
            event = item["event"]
            db_duplicates = self.duplicate_detector.find_duplicates_in_database(event, self.db)
    ...
    def generate_quality_report(self, processed_results: Dict[str, Any]) -> Dict[str, Any]:
        """Generate comprehensive quality report."""
        summary = processed_results["processing_summary"]
        
        # Calculate quality metrics
        total_events = summary["original_count"]
        success_rate = (summary["final_processable_count"] / total_events * 100) if total_events > 0 else 0
        
        # Analyze common quality issues
        issue_frequency = {}
        for item in processed_results["invalid_events"] + processed_results["low_quality_events"]:
        ...
        all_validated = (processed_results["valid_events"] + 
                        processed_results["low_quality_events"] + 
                        processed_results["invalid_events"])
        
        if all_validated:
            total_score = sum(item["validation"]["quality_score"] for item in all_validated)
            report["quality_metrics"]["average_quality_score"] = round(total_score / len(all_validated), 2)
        ...
        # Duplicate rate
        total_duplicates = (summary["batch_duplicates_count"] + summary["db_duplicates_count"])
        ...
        if success_rate < 50:
            report["recommendations"].append("Success rate is very low. Review scraping selectors and data extraction logic.")
        
        if report["quality_metrics"]["average_quality_score"] < 70:
            report["recommendations"].append("Average quality score is low. Improve data cleaning and validation.")
        
        if duplicate_rate > 20:
            report["recommendations"].append("High duplicate rate detected. Review scraping frequency and duplicate detection logic.")
        
        if common_issues:
            report["recommendations"].append(f"Most common issue: '{common_issues[0][0]}'. Focus on fixing this issue first.")
