...
class TranslationService:
    ...
    def get_event_translation(
        self, event_id: int, language_code: str
    ) -> Optional[EventTranslation]:
        """Get event translation for specific language."""
        return (
            self.db.query(EventTranslation)
            .filter(
                and_(
    ...
    def get_category_translation(
        ...
        return (
            self.db.query(CategoryTranslation)
            .filter(
                and_(
    ...
    def get_translated_category(
        self, category: EventCategory, language_code: str
    ) -> Dict[str, Any]:
    ...
    def get_venue_translation(
        self, venue_id: int, language_code: str
    ) -> Optional[VenueTranslation]:
        """Get venue translation for specific language."""
        return (
            self.db.query(VenueTranslation)
            .filter(
                and_(
    ...
    def get_static_content(
        self, key: str, language_code: str, fallback: bool = True
    ) -> Optional[str]:
        """Get translated static content."""
        translation = (
            self.db.query(StaticContentTranslation)
            .filter(
                and_(
    ...
    def get_events_with_translations(
        self, events: List[Event], language_code: str
    ) -> List[Dict[str, Any]]:
    ...
    def get_categories_with_translations(
        self, categories: List[EventCategory], language_code: str
    ) -> List[Dict[str, Any]]:
        """Get multiple categories with translations applied."""
        return [
            self.get_translated_category(category, language_code)
            for category in categories
        ]

    def get_venues_with_translations(
        self, venues: List[Venue], language_code: str
    ) -> List[Dict[str, Any]]:
