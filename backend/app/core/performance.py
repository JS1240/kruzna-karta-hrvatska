...
from .cache import get_cache_service, cached, cache_invalidate_on_change
...
class QueryOptimizer:
    ...
    def optimize_event_query(query, include_analytics: bool = False):
        ...
        if include_analytics:
            query = query.options(
                selectinload(Event.performance_metrics)
            )
        
        return query
    
    @staticmethod
    def add_geographic_optimization(query, latitude: float, longitude: float, radius_km: float):
    ...
    @staticmethod
    def add_date_range_optimization(query, date_from: Optional[date], date_to: Optional[date]):
...
class PerformanceService:
    ...
    def get_event_detail_optimized(self, event_id: int, language: str = "hr") -> Optional[Dict[str, Any]]:
    ...
    def get_popular_events_optimized(self, 
                                   limit: int = 10, 
                                   days: int = 7,
                                   language: str = "hr") -> List[Dict[str, Any]]:
    ...
    def search_events_optimized(self, 
                               query_text: str,
        ...
        events = query.order_by(
            ...
            desc(func.ts_rank(Event.search_vector, func.plainto_tsquery(query_text))),
            Event.date.asc()
        ).offset(skip).limit(size).all()
    ...
    def get_analytics_optimized(self, 
                              date_from: Optional[date] = None,
        ...
        platform_metrics = self.db.query(PlatformMetrics).filter(
            PlatformMetrics.date >= date_from,
            PlatformMetrics.date <= date_to,
            PlatformMetrics.metric_type == 'daily'
        ).order_by(PlatformMetrics.date).all()
        
        # Get event performance metrics
        event_metrics = self.db.query(
        ...
        return {
            ...
            "platform_metrics": [self._serialize_platform_metric(m) for m in platform_metrics],
            "event_summary": {
                "total_views": event_metrics.total_views or 0,
                "unique_views": event_metrics.unique_views or 0,
                "avg_bounce_rate": float(event_metrics.avg_bounce_rate) if event_metrics.avg_bounce_rate else 0
            }
        }
    
    # Cache invalidation methods
    @cache_invalidate_on_change(["events", "event_detail", "popular_events", "search_results"])
    ...
    def update_category(self, category_id: int, update_data: Dict[str, Any]) -> Optional[EventCategory]:
        """Update category with cache invalidation."""
        category = self.db.query(EventCategory).filter(EventCategory.id == category_id).first()
        if category:
            for key, value in update_data.items():
    ...
    def _increment_view_count(self, event_id: int):
        ...
        try:
            self.db.execute(
                text("UPDATE events SET view_count = COALESCE(view_count, 0) + 1 WHERE id = :event_id"),
                {"event_id": event_id}
            )
            self.db.commit()
    ...
    def _serialize_category(self, category: EventCategory, language: str) -> Dict[str, Any]:
