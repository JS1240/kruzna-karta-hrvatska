...
class PostgreSQLBackupService:
    ...
    def __init__(self):
        ...
        self.compression_enabled = (
            os.getenv("BACKUP_COMPRESSION", "true").lower() == "true"
        )
    ...
    def create_full_backup(
        self, custom_filename: Optional[str] = None, include_analytics: bool = True
    ) -> BackupMetadata:
        ...
        try:
            ...
            if self.compression_enabled:
                ...
                with open(temp_path, "wb") as f:
                    ...
                    gzip_proc = subprocess.Popen(
                        ["gzip"], stdin=pg_dump.stdout, stdout=f
                    )
            ...
            if self.compression_enabled:
                metadata.compression_ratio = self._estimate_compression_ratio(
                    backup_path
                )
            ...
            logger.info(
                f"Full backup completed: {backup_path} ({file_size} bytes, {duration:.2f}s)"
            )
            return metadata
    ...
    def restore_from_backup(
        ...
        try:
            ...
            if backup_path.endswith(".gz"):
                ...
                temp_file = (
                    self.temp_dir
                    / f"restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql"
                )
    ...
    def cleanup_old_backups(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Cleanup completed: {len(cleaned_files)} files, {total_size_freed} bytes freed"
            )
    ...
    def verify_backup(self, backup_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            checksum_match = (
                current_checksum == stored_checksum if stored_checksum else None
            )
            ...
            return {"status": "error", "error": str(e), "file_exists": True}
    ...
    def get_backup_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_backups = len(
                [b for b in backups if b["status"] == BackupStatus.COMPLETED.value]
            )
            failed_backups = len(
                [b for b in backups if b["status"] == BackupStatus.FAILED.value]
            )
            corrupted_backups = len(
                [b for b in backups if b["status"] == BackupStatus.CORRUPTED.value]
            )
