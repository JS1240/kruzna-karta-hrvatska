...
class BookingService:
        ...
        try:
            ...
            if quantity < ticket_type.min_purchase:
                raise BookingServiceError(
                    f"Minimum purchase quantity is {ticket_type.min_purchase}"
                )

            if quantity > ticket_type.max_purchase:
                raise BookingServiceError(
                    f"Maximum purchase quantity is {ticket_type.max_purchase}"
                )
            ...
            booking = Booking(
                ...
                expiry_date=now
                + timedelta(minutes=self.default_booking_expiry_minutes),
            )
    ...
    def _process_payment(
        ...
        try:
            ...
            payment.external_payment_id = result.get("payment_intent_id") or result.get(
                "payment_id"
            )
    ...
    def _process_stripe_payment(
        self, payment: Payment, booking: Booking, customer_info: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = stripe_service.create_payment_intent(
                ...
                organizer_id=(
                    getattr(booking.event, "organizer_id", None)
                    if booking.event
                    else None
                ),
            ...
            if result["success"]:
                logger.info(
                    f"Stripe payment intent created for booking {booking.booking_reference}"
                )
                return {
                    "success": True,
                ...
                logger.error(
                    f"Stripe payment failed for booking {booking.booking_reference}: {result}"
                )
                return {
                    "success": False,
    ...
    def _process_credit_card_payment(
        self, payment: Payment, customer_info: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def _sync_with_external_provider(
        self, booking: Booking, action: str
    ) -> Dict[str, Any]:
    ...
    def _sync_with_entrio(self, booking: Booking, action: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Syncing booking {booking.booking_reference} with Entrio - {action}"
        )
    ...
    def _sync_with_eventbrite(self, booking: Booking, action: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Syncing booking {booking.booking_reference} with Eventbrite - {action}"
        )
    ...
    def get_booking(
        self, db: Session, booking_reference: str, user_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """Get booking details."""
        ...
        return {
            "booking": {
                ...
                "ticket_type": (
                    booking.ticket_type.name if booking.ticket_type else None
                ),
                ...
                "confirmation_date": (
                    booking.confirmation_date.isoformat()
                    if booking.confirmation_date
                    else None
                ),
                "expiry_date": (
                    booking.expiry_date.isoformat() if booking.expiry_date else None
                ),
            },
            "tickets": [
                {
                    ...
                    "qr_code": t.qr_code,
                    "check_in_time": (
                        t.check_in_time.isoformat() if t.check_in_time else None
                    ),
                }
                for t in tickets
            ],
    ...
    def cancel_booking(
        self, db: Session, booking_reference: str, user_id: int, reason: str = None
    ) -> Dict[str, Any]:
    ...
    def _process_refund(
        self, db: Session, booking: Booking, reason: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            refund_result = self._process_payment_gateway_refund(
    ...
    def _process_payment_gateway_refund(
        self, payment: Payment, amount: Decimal, reason: str = None
    ) -> Dict[str, Any]:
        """Process refund via payment gateway (Stripe or fallback)."""
        try:
            # Try Stripe refund if payment was processed via Stripe
            if payment.external_payment_id and payment.external_payment_id.startswith(
                ("pi_", "ch_")
            ):
                ...
                logger.info(
                    f"Processing mock refund for payment {payment.payment_reference}"
                )
                return {
                    "success": True,
    ...
    def _process_stripe_refund(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Stripe refund created for payment {payment.payment_reference}"
                )
                return {
                    "success": True,
                ...
                logger.error(
                    f"Stripe refund failed for payment {payment.payment_reference}: {result}"
                )
                return {
                    "success": False,
                    "error": result.get("error", "Stripe refund failed"),
                    "gateway_response": {"status": "failed", "provider": "stripe"},
                }
    ...
    def validate_ticket(
        self, db: Session, ticket_number: str, validation_location: str = None
    ) -> Dict[str, Any]:
        ...
        if ticket.ticket_status != TicketStatus.ACTIVE:
            return {
                "status": "invalid",
                "message": f"Ticket status: {ticket.ticket_status.value}",
            }
        ...
        return {
            ...
            "ticket": {
                ...
                "event_title": (
                    ticket.booking.event.title if ticket.booking.event else None
                ),
                "check_in_time": ticket.check_in_time.isoformat(),
            },
        }
    ...
    def get_booking_statistics(
        self, db: Session, event_id: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        confirmed_bookings = query.filter(
            Booking.booking_status == BookingStatus.CONFIRMED
        ).count()
        paid_bookings = query.filter(
            Booking.booking_status == BookingStatus.PAID
        ).count()
        cancelled_bookings = query.filter(
            Booking.booking_status == BookingStatus.CANCELLED
        ).count()

        # Get revenue statistics
        revenue_query = query.filter(
            Booking.booking_status.in_([BookingStatus.CONFIRMED, BookingStatus.PAID])
        )
        total_revenue = (
            db.execute(
                text(
                    "SELECT COALESCE(SUM(total_price), 0) FROM bookings WHERE booking_status IN ('confirmed', 'paid')"
                )
            ).scalar()
            or 0
        )
        ...
        return {
            ...
            "payment_success_rate": (
                (paid_bookings / total_bookings * 100) if total_bookings > 0 else 0
            ),
        }
