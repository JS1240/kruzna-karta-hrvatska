...
class AnalyticsService:
    ...
    def track_search_click(
        ...
        search_log = self.db.query(SearchLog).filter(SearchLog.id == search_log_id).first()
        if search_log:
    ...
    def aggregate_event_metrics(self, target_date: date) -> None:
        ...
        events_with_views = self.db.query(EventView.event_id).filter(
    ...
    def _aggregate_single_event_metrics(self, event_id: int, target_date: date) -> None:
        ...
        existing = self.db.query(EventPerformanceMetrics).filter(
            and_(
        ...
        metrics.bounce_rate = (bounces / metrics.total_views * 100) if metrics.total_views > 0 else 0
        ...
        metrics.top_countries = dict(sorted(countries.items(), key=lambda x: x[1], reverse=True)[:5])
        metrics.top_cities = dict(sorted(cities.items(), key=lambda x: x[1], reverse=True)[:5])
        ...
        favorites = self.db.query(UserInteraction).filter(
            and_(
        ...
        shares = self.db.query(UserInteraction).filter(
            and_(
                UserInteraction.entity_type == 'event',
                UserInteraction.entity_id == event_id,
    ...
    def aggregate_platform_metrics(self, target_date: date, metric_type: str = 'daily') -> None:
        ...
        metrics.new_users = self.db.query(User).filter(
            func.date(User.created_at) == target_date
        ).count()
        
        # Active users (users who viewed events today)
        metrics.active_users = self.db.query(EventView.user_id).filter(
            and_(
        ...
        # Content metrics
        metrics.total_events = self.db.query(Event).filter(Event.event_status == 'active').count()
        metrics.new_events = self.db.query(Event).filter(
            func.date(Event.created_at) == target_date
        ).count()
        metrics.featured_events = self.db.query(Event).filter(
            and_(Event.is_featured == True, Event.event_status == 'active')
        ).count()
        
        # Engagement metrics
        metrics.total_page_views = self.db.query(EventView).filter(
            func.date(EventView.viewed_at) == target_date
        ).count()
        
        metrics.total_searches = self.db.query(SearchLog).filter(
            func.date(SearchLog.searched_at) == target_date
        ).count()
        
        # Performance indicators
        total_views = self.db.query(EventView).filter(
            func.date(EventView.viewed_at) == target_date
        ).count()
        
        bounces = self.db.query(EventView).filter(
            and_(
                func.date(EventView.viewed_at) == target_date,
        ...
        # Search success rate
        successful_searches = self.db.query(SearchLog).filter(
            and_(
                func.date(SearchLog.searched_at) == target_date,
        ...
        metrics.search_success_rate = (successful_searches / metrics.total_searches * 100) if metrics.total_searches > 0 else 0
    ...
    def get_event_analytics(
        ...
        metrics = self.db.query(EventPerformanceMetrics).filter(
            and_(
                EventPerformanceMetrics.event_id == event_id,
                EventPerformanceMetrics.date >= date_from,
                EventPerformanceMetrics.date <= date_to
            )
        ).order_by(EventPerformanceMetrics.date).all()
        ...
        # Calculate averages
        avg_bounce_rate = sum(m.bounce_rate for m in metrics) / len(metrics) if metrics else 0
        avg_view_duration = sum(m.avg_view_duration for m in metrics) / len(metrics) if metrics else 0
    ...
    def get_search_analytics(
        ...
        ).group_by(SearchLog.query).order_by(desc('search_count')).limit(20).all()
