...
from ..models.recurring_events import (
    EventSeries, EventInstance, SeriesModification, InstanceModification,
    RecurrenceRule, SeriesTemplate, RecurrencePattern, RecurrenceEndType,
    SeriesStatus, EventInstanceStatus
)
...
class RecurringEventsService:
    ...
    def create_event_series(
        ...
        try:
            ...
            series = EventSeries(
                ...
                recurrence_end_type=RecurrenceEndType(series_data.get("recurrence_end_type", "never")),
                ...
                advance_notice_days=series_data.get("advance_notice_days", self.default_advance_notice_days)
            )
            ...
            return {
                ...
                "series": {
                    ...
                    "next_instance_date": instances[0].scheduled_date.isoformat() if instances else None
                },
    ...
    def _validate_recurrence_pattern(self, series_data: Dict[str, Any]) -> None:
        ...
        if pattern == "weekly" and not series_data.get("recurrence_days"):
            raise RecurringEventsServiceError("Weekly recurrence requires recurrence_days")
        
        if series_data.get("recurrence_end_type") == "after_occurrences":
            if not series_data.get("max_occurrences"):
                raise RecurringEventsServiceError("max_occurrences required for 'after_occurrences' end type")
        
        if series_data.get("recurrence_end_type") == "on_date":
            if not series_data.get("recurrence_end_date"):
                raise RecurringEventsServiceError("recurrence_end_date required for 'on_date' end type")
    
    def _generate_instances(self, db: Session, series: EventSeries) -> List[EventInstance]:
        ...
        occurrence_dates = self._calculate_occurrence_dates(series, start_date, end_date)
        
        # Create instances
        for i, occurrence_date in enumerate(occurrence_dates[:self.max_instances_per_generation]):
            instance = EventInstance(
                ...
                sequence_number=i + 1,
                instance_status=EventInstanceStatus.PUBLISHED if series.auto_publish else EventInstanceStatus.SCHEDULED
            )
    ...
    def _calculate_occurrence_dates(self, series: EventSeries, start_date: date, end_date: date) -> List[date]:
        ...
        # Add end condition
        if series.recurrence_end_type == RecurrenceEndType.ON_DATE:
            rrule_kwargs['until'] = datetime.combine(series.recurrence_end_date, datetime.max.time())
        elif series.recurrence_end_type == RecurrenceEndType.AFTER_OCCURRENCES:
            rrule_kwargs['count'] = series.max_occurrences
        
        # Add weekday restrictions for weekly/biweekly
        if series.recurrence_pattern in [RecurrencePattern.WEEKLY, RecurrencePattern.BIWEEKLY]:
        ...
        try:
            ...
            if series.excluded_dates:
                excluded = {datetime.fromisoformat(d).date() for d in series.excluded_dates}
                occurrence_dates = [d for d in occurrence_dates if d not in excluded]
            
            # Add inclusions
            if series.included_dates:
                included = {datetime.fromisoformat(d).date() for d in series.included_dates}
    ...
    def _calculate_custom_occurrences(self, series: EventSeries, start_date: date, end_date: date) -> List[date]:
    ...
    def _publish_instance(self, db: Session, instance: EventInstance) -> None:
        ...
        try:
            ...
            event = Event(
                ...
                organizer=instance.series.organizer.username if instance.series.organizer else None,
    ...
    def _apply_series_changes_to_instances(
        ...
        for instance in instances:
            ...
            for field, new_value in update_data.items():
                ...
                if (hasattr(instance, instance_field) and 
                    hasattr(instance, override_field) and
                    not getattr(instance, override_field, False)):
    ...
    def _regenerate_future_instances(self, db: Session, series: EventSeries) -> None:
        ...
        future_instances = db.query(EventInstance).filter(
        ...
        # Update series statistics
        series.total_instances = db.query(EventInstance).filter(
            EventInstance.series_id == series.id
        ).count()
        series.last_generated_at = datetime.now()
    ...
    def modify_instance(
        ...
        try:
            instance = db.query(EventInstance).filter(EventInstance.id == instance_id).first()
            if not instance:
    ...
    def cancel_instance(self, db: Session, instance_id: int, user_id: int, reason: str = None) -> Dict[str, Any]:
    ...
    def get_series_instances(
        ...
        if status_filter:
            try:
                ...
                raise RecurringEventsServiceError(f"Invalid status filter: {status_filter}")
        ...
        instances = query.order_by(EventInstance.scheduled_date.asc()).offset(skip).limit(size).all()
    ...
    def get_upcoming_instances(
        ...
        instances = query.order_by(EventInstance.scheduled_date.asc()).limit(limit).all()
    ...
    def generate_series_statistics(self, db: Session, series_id: int) -> Dict[str, Any]:
        ...
        # Instance statistics
        total_instances = db.query(EventInstance).filter(EventInstance.series_id == series_id).count()
        
        published_instances = db.query(EventInstance).filter(
            EventInstance.series_id == series_id,
            EventInstance.instance_status == EventInstanceStatus.PUBLISHED
        ).count()
        
        cancelled_instances = db.query(EventInstance).filter(
            EventInstance.series_id == series_id,
            EventInstance.instance_status == EventInstanceStatus.CANCELLED
        ).count()
        
        completed_instances = db.query(EventInstance).filter(
            EventInstance.series_id == series_id,
            EventInstance.instance_status == EventInstanceStatus.COMPLETED
        ).count()
        
        # Next instance
        next_instance = db.query(EventInstance).filter(
            EventInstance.series_id == series_id,
            EventInstance.scheduled_date >= date.today(),
            EventInstance.instance_status.in_([
                EventInstanceStatus.SCHEDULED,
        ...
        ).order_by(EventInstance.scheduled_date.asc()).first()
        ...
        return {
            ...
            "statistics": {
                ...
                "cancellation_rate": (cancelled_instances / total_instances * 100) if total_instances > 0 else 0
            },
            "next_instance": {
