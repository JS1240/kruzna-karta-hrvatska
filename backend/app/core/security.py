...
class SecurityHardening:
    ...
    def check_data_access_permissions(
        self, user_id: int, table_name: str, operation: str
...
class GDPRCompliance:
        ...
        try:
            ...
            if user_result:
                user_data["profile"] = {
                    "id": user_result.id,
                    "email": (
                        self.security.decrypt_sensitive_data(user_result.email)
                    ...
                    "phone_number": (
                        self.security.decrypt_sensitive_data(user_result.phone_number)
                        if user_result.phone_number
                        else None
                    ),
                    "date_of_birth": (
                        user_result.date_of_birth.isoformat()
                        if user_result.date_of_birth
                        else None
                    ),
                    "created_at": user_result.created_at.isoformat(),
                    "last_login": (
                        user_result.last_login.isoformat()
                        if user_result.last_login
                        else None
                    ),
                }
            ...
            user_data["event_views"] = [
                {"event_id": view.event_id, "timestamp": view.viewed_at.isoformat()}
                for view in views
            ]
            ...
            return {"status": "error", "message": f"Access request failed: {str(e)}"}
    ...
    def _handle_erasure_request(self, request: DataSubjectRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "message": f"Erasure request failed: {str(e)}"}
    ...
    def _check_erasure_legality(self, user_id: int, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            if legal_holds and legal_holds.count > 0:
                return {"can_erase": False, "reason": "Data subject to legal hold"}

            return {"can_erase": True, "reason": None}

        except Exception as e:
            logger.error(f"Failed to check erasure legality: {e}")
            return {"can_erase": False, "reason": f"Legal check failed: {str(e)}"}
    ...
    def run_data_retention_cleanup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cleanup_actions.append(
                f"Anonymized {result.rowcount} old audit log entries"
            )
